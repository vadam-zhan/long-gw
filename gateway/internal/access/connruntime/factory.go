...
func (f *Factory) CreateAndRun(ctx context.Context, tp transport.Transport) {
	...
	if kicked != nil {
		kicked.Close(&gatewayv1.KickRequest{Code: 4011, Reason: "replaced by new login"})
	}
	...
	conn.Run(
		func(c *Connection, msg *gatewayv1.Message) {
...
func (f *Factory) sendAuthResponse(
	ctx context.Context,
	conn *Connection,
	ref *gatewayv1.Message,
	ai *authInfo,
	sessionID string,
	lastSeq uint64,
) error {
	resp := &gatewayv1.Message{
		Type:  gatewayv1.FrameType_HANDSHAKE_ACK,
		MsgId: ref.MsgId,
		TraceContext: &gatewayv1.TraceContext{
			TraceId: ref.TraceContext.TraceId,
		},
	}
...
func (f *Factory) doHandshake(ctx context.Context, tp transport.Transport) (auth *authInfo, msg *gatewayv1.Message, err error) {
	...
	var handshakeRequest gatewayv1.HandshakeRequest
	if err := proto.Unmarshal(msg.GetBody().GetData(), &handshakeRequest); err != nil {
	...
	userID, deviceID, err := f.deps.HandlerReg.AuthVerifier().Verify(handshakeRequest.Token)
	if err != nil {
	...
	return &authInfo{
		userID:     userID,
		deviceID:   deviceID,
		appID:      handshakeRequest.AppId,
		deviceType: handshakeRequest.DeviceType,
		bizCode:    msg.BizCode,
