...
type ImplRegistry struct {
	mu           sync.RWMutex
	handlers     map[gateway.FrameType]types.MsgHandler
...
func Build(av types.AuthVerifier, chain types.UplinkChain) *ImplRegistry {
	reg := NewRegistry(av)
	reg.Register(gateway.FrameType_PING, &HeartbeatHandler{})
	reg.Register(gateway.FrameType_KICK, &LogoutHandler{})
	reg.Register(gateway.FrameType_BUSINESS, &UpstreamHandler{chain: chain})
	return reg
}
...
func (r *ImplRegistry) Register(msgType gateway.FrameType, handler types.MsgHandler) {
...
func replyError(conn types.ConnSubmitter, ref *gateway.Message, code int32, text string) {
	errMsg := &gateway.Message{
		...
		MsgId: ref.MsgId,
		Body: &gateway.Body{
			Data: []byte(fmt.Sprintf(`{"code":%d,"message":"%s"}`, code, text)),
		},
	}
